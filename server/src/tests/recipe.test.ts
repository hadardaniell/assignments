...
describe("Recipes Module Integration Tests", () => {
    ...
    it("should get recipe by Id", async () => {
        const res = await request(app)
            .get(`/api/recipes/getRecipeById/${createdRecipeId}`)
            .set(authHeader());
    ...
    it("should delete recipe", async () => {
        const res = await request(app)
            .delete(`/api/recipes/deleteRecipe/${createdRecipeId}`)
            .set(authHeader());
        ...
        expect([404, 401]).toContain(check.status);
    });
